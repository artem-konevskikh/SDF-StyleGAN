...
def generate(
    ...
    for i in tqdm(range(num_generate)):
        ...
        while mesh is None:
            mesh = model.generate_mesh(
                ema=ema, mc_vol_size=mc_vol_size,
                level=level, trunc_psi=trunc_psi
            )
        ...
        image = render_mesh(mesh, index=index, resolution=render_resolution)
        image = image / 255
    ...
    torchvision.utils.save_image(
        ...
        nrow=min(math.ceil(np.sqrt(num_generate)), 15),
    )
...
def generate_mesh(
    ...
    for i in tqdm(range(num_generate)):
        ...
        mesh = scale_to_unit_sphere(mesh)
        name = f"/generate_{model_name}_{model_id}_{ema}_{level}_{mc_vol_size}"
        name = f"{name}_{trunc_psi}_{i}.obj"
...
def generate_interpolation(
    ...
    for i in tqdm(range(rows)):
        ...
        ratios = torch.linspace(0.0, 1.0, interpolation_num_steps)
        for j in range(interpolation_num_steps):
            ...
            sdf = (
                model
                .ME(points, generated_voxels)
                .view(vol_size, vol_size, vol_size)
                - level
            )
            ...
            image = (
                render_mesh(mesh, index=view_index,
                            resolution=render_resolution) / 255
            )
            image = torch.from_numpy(image.copy()).permute(2, 0, 1)
            images.append(image)

    name = f"interpolation_{rows}_{interpolation_num_steps}_{model_name}"
    name = f"{name}_{model_id}_{ema}_{level}_{trunc_psi}_{view_index}"
    torchvision.utils.save_image(
...
def interpolate_mesh(
    ...
    ratios = torch.linspace(0.0, 1.0, interpolation_num_steps)
    for j in tqdm(range(interpolation_num_steps)):
        ratio = ratios[j]
        interp_latents = linear_slerp(ratio, latents_low, latents_high)
        latents = [(interp_latents, model.num_layers)]
        points = model.get_voxel_coordinates(resolution=vol_size)
        ...
        sdf = (
            model.ME(points, generated_voxels)
            .view(vol_size, vol_size, vol_size)
            - level
        )

        mesh = scale_to_unit_sphere(process_sdf(sdf.cpu().numpy(),
                                                level=level))
        mesh_name = f"/interpolate_mesh_{model_name}_{model_id}_{ema}_{level}"
        mesh_name = f"{mesh_name}_{vol_size}_{trunc_psi}_{j}.obj"
